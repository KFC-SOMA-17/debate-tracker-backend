...
- 메인 런타임은 **MySQL** (`runtimeOnly 'com.mysql:mysql-connector-j'`, 로컬은 Docker Compose). 테스트도 **MySQL** 로 일원화 — 로컬은 떠 있는 MySQL(`debate_test` DB)/Redis(번호 DB 1) 에 접속(`test` 프로파일, 기본값), CI 는 Testcontainers(`ci` 프로파일, `SPRING_PROFILES_ACTIVE=ci`). 테스트 DB 정리는 `DatabaseCleaner`(MySQL `TRUNCATE` 기반). Flyway 도입 전까진 `ddl-auto: create-drop`.
